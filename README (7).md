...
- [1. Spin up the EKS cluster](#1-spin-up-the-eks-cluster)
- [2. Turn on EBS storage (needed for Postgres + workspace home dirs)](#2-turn-on-ebs-storage-needed-for-postgres--workspace-home-dirs)
- [3. Set up the sandbox namespace + scoped RBAC](#3-set-up-the-sandbox-namespace--scoped-rbac)
- [4. Stand up Postgres (Coder's database)](#4-stand-up-postgres-coders-database)
- [5. Deploy Coder](#5-deploy-coder)
- [6. Set up your admin login + UI access](#6-set-up-your-admin-login--ui-access)
- [7. Get the Coder CLI installed (no sudo needed)](#7-get-the-coder-cli-installed-no-sudo-needed)
- [8. Publish the scoped workspace template](#8-publish-the-scoped-workspace-template)
- [9. Spin up a workspace](#9-spin-up-a-workspace)
- [10. Confirm the sandbox actually holds](#10-confirm-the-sandbox-actually-holds)
- [11. Turn on the Agents/Tasks chat tab (Claude Code)](#11-turn-on-the-agentstasks-chat-tab-claude-code)
- [12. Jump into the workspace](#12-jump-into-the-workspace)
- [13. Drop the sample app into the workspace](#13-drop-the-sample-app-into-the-workspace)
- [14. Agent prompts — run these inside the workspace, in order](#14-agent-prompts--run-these-inside-the-workspace-in-order)
- [Teardown (avoid ongoing AWS cost)](#teardown-avoid-ongoing-aws-cost)
...
- Carries `CODER_ACCESS_URL`/`CODER_WILDCARD_ACCESS_URL` values from the
  **current live deployment** (`35-174-85-96.nip.io`). On a fresh cluster
  ...
  below.
- Sets `CODER_OAUTH2_GITHUB_DEFAULT_PROVIDER_ENABLE: "false"` to remove
...
`CODER_ACCESS_URL` / `CODER_WILDCARD_ACCESS_URL` values for the nip.io
...
**Open the UI**: browse to `$ACCESS_URL` and log in through the
...
`os.environ["APP_MODE"]` on startup with no default, so if the first
